...
## 模式
- **985吧·实时锐评** —— 真实金句卡片流,按主题筛选:`专业鄙视链 / 院校battle / 排行榜 / 就业出口 / 升学保研 / 出国留学 / 科研读博 / 分数线跳水 / 招生套路 / 后悔实录`。含**招生风格图鉴**(超短裙 / 姜-泰勒 / 共轭 + 2026今年新动向)、**黑话解码**、**🎲 随机斗一只蛐蛐**;部分卡片嵌 **9吧原图**(榜单/录用名单)和可折叠的 **🦗 楼中对线**。
...
```
> 密钥只从环境变量读,**绝不写入仓库**(`.env` 已 gitignore)。**想问答快**:推理模型(gpt-5.x-high)首字要十几~几十秒;把 `LLM_MODEL` 设成快的流式模型(如 `DeepSeek-V3.2` / `chatgpt-4o-latest` / `gemini-2.5-flash`)首字 2~10 秒。对话记忆存浏览器 localStorage(切板块/刷新不丢,可一键清空)。
